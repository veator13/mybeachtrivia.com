...
function setTab(tab) {
  ...
  // ----- NEW: redeem invite if present -----
  async function redeemInviteIfAny(user) {
    if (!user) return;
    let inviteId = null;
    try {
    ...
    try {
      const inviteRef = db.collection("employeeInvites").doc(inviteId);
      const snap = await inviteRef.get();
      if (!snap.exists) return; // already used/revoked

      const invite = snap.data() || {};
      const userEmail   = (user.email || "").toLowerCase();
      const inviteEmail = (invite.email || "").toLowerCase();

      // Require email match, as enforced by Firestore rules
      if (userEmail !== inviteEmail) {
        console.warn("[invite] Email mismatch; not redeeming.");
        return;
      }

      // Create/merge employees/{uid}; aligns with the self-register rules path
      const empRef = db.collection("employees").doc(user.uid);
      await empRef.set({
        uid: user.uid,
        email: user.email || "",
        firstName: "",
      ...
      // Delete invite (rules let invited email delete their own)
      await inviteRef.delete();

      // Clean ?invite= from URL so refreshes don't re-run redemption
      const clean = new URL(location.href);
      clean.searchParams.delete("invite");
      history.replaceState({}, "", clean.href);

      console.log("[invite] Redeemed & deleted invite:", inviteId);
    } catch (e) {
  ...
  async function authorizeAndRedirect(user) {
    ...
    // First: redeem invite (if the URL had one) so the employee doc exists right away
    await redeemInviteIfAny(user);
    ...
    requireRoleForSelection(Array.isArray(employee.roles) ? employee.roles : []);
    ...
    try {
      ...
      if (t) {
        ...
        if (u.origin === location.origin) {
          sessionStorage.removeItem("afterLogin");
          console.log("[login.js] redirecting to afterLogin =", u.href);
          location.replace(u.href);
...
// Prefer sessionStorage.afterLogin if present (same-origin) and EXIT this handler
try {
  var __t = sessionStorage.getItem("afterLogin");
  if (__t) {
    var __u = new URL(__t, location.origin);
    if (__u.origin === location.origin) {
      sessionStorage.removeItem("afterLogin");
      console.log("[login.js] redirecting to afterLogin =", __u.href);
...
try {
  const __snap = await db.collection('employees').doc(user.uid).get({ source: 'server' });
  const __d    = __snap.exists ? (__snap.data() || {}) : {};
  const __incomplete =
    __d.onboardingComplete !== true ||
    !(__d.firstName && __d.lastName) ||
    !(__d.phone);

  if (__incomplete || __d.active !== true) {
    console.log("[login.js] routing to onboarding (incomplete or inactive profile)");
