// auth-guard.js — Protect Venue Reports page (Firebase compat SDK)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function showError(message) {
      console.error('[venue-reports auth-guard] ' + message);
    ...
    auth.onAuthStateChanged(async (user) => {
      try {
        ...
        if (!roles.includes('admin')) {
          showError('Admin access is required to view Venue Reports.');
          return;
        }

        console.log('[venue-reports auth-guard] Admin access verified for:', user.email || user.uid);
        ...
        window.__VENUE_REPORTS_AUTH_READY__ = true;
        window.__VENUE_REPORTS_USER__ = user;
        window.__VENUE_REPORTS_EMPLOYEE__ = emp;

        document.dispatchEvent(new CustomEvent('venue-reports:auth-ready', {
          detail: { user, emp }
        }));
      } catch (err) {
        console.error('[venue-reports auth-guard] verification error:', err);
