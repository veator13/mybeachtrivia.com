...
/* =========================
 *  SINGLE-SHIFT MOVE (WITH CONFLICT / OVERRIDE)
 * ========================= */

/**
 * Move a single shift to a new date.
 * This is what your drag/drop handler should call.
 *
 * - Does a double-booking check by default.
 * - Treats "Unknown host" (or missing employee) as NON-blocking.
 * - Call again with { ignoreConflicts: true } after "Proceed anyway".
 *
 * @param {string|number} shiftId
 * @param {string} targetDateYMD - YYYY-MM-DD
 * @param {Object} options
 * @param {boolean} [options.ignoreConflicts=false]  If true, skips the conflict check.
 * @returns {Promise<{ok: boolean, reason?: 'not-found'|'conflict'|'error', conflictShift?: Object, updatedShift?: Object}>}
 */
async function moveSingleShiftToDate(shiftId, targetDateYMD, options = {}) {
  const { ignoreConflicts = false } = options;

  if (!Array.isArray(shifts)) {
    console.error('moveSingleShiftToDate: global "shifts" array is not available');
  ...
  if (idx === -1) {
    console.warn('moveSingleShiftToDate: shift not found for id', shiftId);
    return { ok: false, reason: 'not-found' };
  }

  const original = shifts[idx];
  const updatedShift = { ...original, date: targetDateYMD };

  if (!ignoreConflicts) {
    const conflict = _findShiftConflict(updatedShift, { ignoreUnknownHosts: true });
    if (conflict) {
      // Let the caller open the conflict modal and stash the pending move.
      return {
        ok: false,
        reason: 'conflict',
  ...
  try {
    await updateShiftInFirebase(String(original.id), { date: targetDateYMD });
    ...
    console.error('moveSingleShiftToDate: failed to update Firebase', err);
    return {
      ok: false,
      reason: 'error'
    };
...
/**
 * Returns the FIRST conflicting shift object if any, otherwise null.
 *
 * "Conflict" here means:
 *  - Same date
 *  - Same employee (host)
 *  - Time ranges overlap
 *
 * If ignoreUnknownHosts is true:
 *  - A candidate with no real employee (missing, null, or "Unknown host") is NOT blocked.
 *  - Existing shifts with unknown hosts are ignored as blockers.
 */
function _findShiftConflict(candidateShift, { ignoreUnknownHosts = true } = {}) {
  if (!candidateShift || !candidateShift.date) return null;
  if (!Array.isArray(shifts)) return null;

  const candidateEmpId = candidateShift.employeeId;

  // If we're ignoring unknown hosts and THIS shift is unknown, don't block it at all.
  if (ignoreUnknownHosts) {
    const name = employees && employees[candidateEmpId];
    if (!candidateEmpId || !name || name === 'Unknown host') {
      return null;
    }
  }

  const candidateStart = _parseTimeToMinutes(candidateShift.startTime);
  const candidateEnd = _parseTimeToMinutes(candidateShift.endTime);

  for (const s of shifts) {
    if (!s || !s.date) continue;

    // Skip the same record
    if (String(s.id) === String(candidateShift.id)) continue;
    if (s.date !== candidateShift.date) continue;

    // Only compare shifts for the same known host (if we're ignoring unknown hosts)
    if (ignoreUnknownHosts) {
      const otherName = employees && employees[s.employeeId];
      if (!s.employeeId || !otherName || otherName === 'Unknown host') {
        continue;
      }
    }

    if (s.employeeId !== candidateEmpId) continue;

    const sStart = _parseTimeToMinutes(s.startTime);
    const sEnd = _parseTimeToMinutes(s.endTime);

    // Basic overlap: start < otherEnd && end > otherStart
 ...
 * Returns 0 for invalid/empty strings so we don't explode.
 */
