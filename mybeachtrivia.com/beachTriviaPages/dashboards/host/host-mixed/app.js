// host-mixed/app.js — Mixed Events host console (all published shows, all types)
...
(function () {
  ...
  var SHOW_TYPE_LABELS = {
    'feud':           'Feud',
    'themed-trivia':  'Themed',
    'classic-trivia': 'Classic',
  };
  ...
  const state = {
    ...
    revealed: false,
    live: false,
    sessionCode: 'MX-' + Math.floor(1000 + Math.random() * 9000),
  };
  ...
  function loadShows() {
    firebase.firestore().collection('publishedShows')
      ...
      .catch(function (err) {
        console.error('[host-mixed] loadShows:', err);
        show('show-picker', 'flex');
      });
  }
  ...
  function populatePicker() {
    ...
    // All shows — no filter; label each with its type
    ...
    state.shows.forEach(function (s) {
      ...
      const showType = (s.show && s.show.showType) || 'classic-trivia';
      const typeTag  = SHOW_TYPE_LABELS[showType] ? '  [' + SHOW_TYPE_LABELS[showType] + ']' : '';
      opt.textContent = title + date + typeTag;
      sel.appendChild(opt);
    });
  ...
  function loadSelectedShow() {
    ...
    firebase.firestore().collection('publishedShows').doc(id).get()
      ...
      .catch(function (err) {
        console.error('[host-mixed] loadSelectedShow:', err);
  ...
  function pushState() {
    ...
    }).catch(function (err) { console.error('[host-mixed] pushState:', err); });
  }
