// host-themed-trivia/app.js — Themed Trivia host console (filters showType === "themed-trivia")
...
(function () {
  ...
  const state = {
    ...
    revealed: false,
    live: false,
    sessionCode: 'TT-' + Math.floor(1000 + Math.random() * 9000),
  };
  ...
  function loadShows() {
    firebase.firestore().collection('publishedShows')
      ...
      .catch(function (err) {
        console.error('[host-themed-trivia] loadShows:', err);
        show('show-picker', 'flex');
      });
  }
  ...
  function populatePicker() {
    ...
    const filtered = state.shows.filter(function (s) {
      var t = (s.show && s.show.showType) || '';
      return t === 'themed-trivia';
    });

    if (!filtered.length) {
      sel.innerHTML = '<option value="">No published Themed Trivia shows found</option>';
      return;
    }

    const blank = document.createElement('option');
    blank.value = '';
    blank.textContent = '— Select a themed show —';
    ...
    const today = filtered.find(matchesToday);
    if (today) { sel.value = today.__id; dom['btn-load-show'].disabled = false; }
  }
  ...
  function loadSelectedShow() {
    ...
    firebase.firestore().collection('publishedShows').doc(id).get()
      ...
      .catch(function (err) {
        console.error('[host-themed-trivia] loadSelectedShow:', err);
    ...
    blocks.forEach(function (entry) {
      ...
      (Array.isArray(block.slides) ? block.slides : []).forEach(function (s) {
        ...
        slides.push({
          ...
          answer:        s.answer      || '',
          notes:         s.notes       || '',
          theme:         s.themeStyle  || block.themeStyle || 'Themed Trivia',
  ...
  function updateRevealChrome() {
    ...
    if (dom['btn-host-hide']) dom['btn-host-hide'].style.display = hostOk ? '' : 'none';
    ...
    if (dom['btn-hide']) dom['btn-hide'].style.display = castOk ? '' : 'none';
  }
  ...
  function setHostRevealed(val) { state.hostRevealed = val; renderHost(); }

  function toggleHostReveal() {
    if (!slideRevealApplicable(state.deck.slides[state.hostSlide] || null)) return;
  ...
  function setRevealed(val) { state.revealed = val; renderCast(); pushState(); }

  function toggleCastReveal() {
    if (!slideRevealApplicable(state.deck.slides[state.castSlide] || null)) return;
  ...
  function updateLiveButton() {
    ...
    if (!btn) return;
    btn.textContent = state.live ? '● Live' : 'Go Live';
  ...
  function pushState() {
    ...
    firebase.firestore().collection('liveSessions').doc(state.sessionCode).set({
      ...
      currentSlide: {
        ...
        questionFontScale: typeof slide.questionFontScale === 'number' ? slide.questionFontScale : 1,
    ...
    }).catch(function (err) { console.error('[host-themed-trivia] pushState:', err); });
  }

  function show(id, display) { const el = dom[id] || document.getElementById(id); if (el) el.style.display = display || 'block'; }
  function hide(id) { const el = dom[id] || document.getElementById(id); if (el) el.style.display = 'none'; }
  function setText(id, val) { const el = dom[id] || document.getElementById(id); if (el) el.textContent = val == null ? '' : String(val); }
  function on(id, evt, fn) { const el = dom[id] || document.getElementById(id); if (el) el.addEventListener(evt, fn); }
